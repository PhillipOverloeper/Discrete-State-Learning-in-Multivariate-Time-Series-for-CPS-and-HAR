...
class MHEALTH_Dataloader:
    ...
    def __init__(
        ...
        if filenames is None:
            self.filenames = [f"data{i}.csv" for i in range(1, 12)]
        else:
    ...
    def _add_subjects(self, data_list):
        ...
        for i, df in enumerate(data_list):
            df.insert(len(df.columns), "subject", i + 1)
    ...
    def save_processed_data(self, data_dict, folder = None):
        ...
        save_split("train", data_dict, folder, "mhealth")
        save_split("val", data_dict, folder, "mhealth")
        save_split("test", data_dict, folder, "mhealth")
    ...
    def load_processed_data(
        ...
        def load_split(split_name: str):
            path = os.path.join(folder, f"mhealth_{split_name}_windows.npz")
    ...
    def _prepare_mhealth_datasets(self):
        """
        ...
        subjects = all_data[len(all_data.columns)-1].dropna().unique()
        all_data = all_data.loc[all_data[len(all_data.columns)-2] != 0]
        ...
        train_raw = all_data[all_data[len(all_data.columns)-1].isin(train_subjects)].reset_index(drop=True)
        test_df = all_data[all_data[len(all_data.columns)-1].isin(test_subjects)].reset_index(drop=True)
        ...
        subjects = train_raw[len(train_raw.columns)-1].dropna().unique()
        n_subj = len(subjects)
        ...
        train_df = all_data[all_data[len(all_data.columns)-1].isin(train_subjects)].reset_index(drop=True)
        val_df = all_data[all_data[len(all_data.columns)-1].isin(val_subjects)].reset_index(drop=True)
        ...
        def windowize_by_run(df):
            ...
            for rk, g in df.groupby(len(df.columns)-1, sort=False):
                ...
                # Target y is the 2nd to last column (index -2)
                y = g[len(g.columns)-2].to_numpy()
...
#loader = MHEALTH_Dataloader()

#out = loader._prepare_mhealth_datasets()
